...
export class MapComponent implements OnInit, AfterViewInit {
  ...
  private initializeMap(): void {
    ...
    const features = countries.map(country => {
      ...
      feature.setStyle(new Style({
        image: new Icon({
          src: "marker-rosso-icona.png", // Cambia con il percorso della tua icona
          scale: 0.05 // Regola la dimensione
        })
      }));
  ...
  private openPopupGrid(countryName: string, coordinate: any): void {
    ...
    if (popupElement) {
	  ...
	  popupElement.style.width = '400px';   // Imposta la larghezza direttamente
	  popupElement.style.height = 'auto';   // Imposta l'altezza in modo dinamico

      this.map?.addOverlay(popupOverlay);

      // Crea il componente popup-grid dinamicamente
